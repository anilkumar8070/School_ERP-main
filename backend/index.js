...
async function sendMail({
  ...
  try {
    ...
    if (!sendgridApiOnly && smtpHost && smtpPort && smtpUser && smtpPass) {
      ...
      const smtpTlsReject = typeof process.env.SMTP_TLS_REJECT_UNAUTHORIZED !== 'undefined' ? String(process.env.SMTP_TLS_REJECT_UNAUTHORIZED).toLowerCase() !== 'false' : process.env.NODE_ENV === 'production';
      ...
      transporter = nodemailer.createTransport({
        ...
        tls: {
          rejectUnauthorized: smtpTlsReject
        },
        logger: smtpDebug,
        debug: smtpDebug
      });
      ...
      try {
        ...
        if (attachments && Array.isArray(attachments) && attachments.length > 0) {
          msg.attachments = attachments.map(a => {
            ...
            return {
              content,
              filename: a.filename || (a.path ? a.path.split('/').pop() : 'attachment'),
              type: a.contentType || a.contentType || a.contentType || a.mime || undefined,
        ...
        try {
          ...
          try {
            if (e && e.stack) console.warn('SendGrid error stack:', e.stack);
          } catch (le) {}
