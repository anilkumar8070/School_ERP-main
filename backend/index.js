...
app.post('/api/faculty/register', async (req, res) => {
  if (!dbConnected) return res.status(503).json({ message: 'Database not available' })
  try {
    const { name, email, subject, education, contact, avatar, experience, classGrade, houses, password } = req.body || {}
    ...
    const reg = await FacultyRegistration.create({ name, email, subject, education, contact, avatar, experience, classGrade, houses: Array.isArray(houses) ? houses : [], password, status: 'pending' })
