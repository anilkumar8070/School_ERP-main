...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    if (!cls) {
      const items = await prisma.syllabus.findMany().sort({
        uploadedAt: -1
      });
    ...
    const items = await prisma.syllabus.findMany({ where: q }).sort({
      uploadedAt: -1
    });
    return res.json(items);
  } catch (e) {
...
router.delete("/:id", verifyToken, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    const doc = await prisma.syllabus.findUnique({ where: { id: String(id) } });
