...
function UserMeetings() {
    ...
    if (!meetings || meetings.length === 0) return <div className="meetings-empty">No meetings found.</div>
                        ...
                        {(() => {
                            try {
                                ...
                                if (dt.getTime() <= now.getTime()) {
                                    return <span className="mi-expired">Expired</span>
                                }
                            } catch (e) { }
                            if (m.link) return <a className="mi-join" href={m.link} target="_blank" rel="noreferrer">Join</a>
                            return <span className="mi-no-link">No link</span>
                        })()}
                    </div>
                </div>
            ))}
